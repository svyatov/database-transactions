...
def run(s, t):
    ...
    eq(
        held,
        [
            {"object_name": "accounts", "index_name": None, "lock_type": "TABLE", "lock_mode": "IX", "lock_status": "GRANTED", "lock_data": None},
    ...
    chain = M("SELECT waiting_pid, blocking_pid FROM sys.innodb_lock_waits")
...
scenario = Scenario(
    ...
    claim="data_locks shows every lock a transaction holds — a single-row UPDATE takes an intention lock on the table plus a record lock on the row — and a waiter shows up as WAITING, findable via sys.innodb_lock_waits.",
    setup="""
