...
VACUUM is the collector for the garbage [MVCC leaves behind](/04-mvcc/dead-tuples-and-bloat):
...
<<< ../../scenarios/04-mvcc/vacuum.ts#demo{ts}
...
<<< ../../scenarios/04-mvcc/vacuum.ts#size{ts}
...
but it holds [`ACCESS EXCLUSIVE`](/03-locking/table-locks-and-ddl) for the whole rewrite:
...
- A stable, bloated-but-recycling table is fine: new versions reuse freed slots, and size
  ...
  [long transaction](/04-mvcc/long-transactions) pinning the horizon.
