...
(often fast) work. The disaster needs one more ingredient — the [lock queue](/03-locking/lock-queues).

## The outage, reproduced

<<< ../../scenarios/03-locking/alter-table-outage.ts#demo{ts}
...
<<< ../../scenarios/03-locking/ddl-lock-timeout.ts#demo{ts}
...
- Watching it live: the wait-chain query from the transcript is unpacked in
  [monitoring locks](/03-locking/monitoring-locks).
