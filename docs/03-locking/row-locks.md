...
<<< ../../scenarios/03-locking/for-update-blocks.ts#demo{ts}
...
<<< ../../scenarios/03-locking/lock-mode-matrix.ts#demo{ts}
...
<<< ../../scenarios/03-locking/fk-key-share.ts#demo{ts}
...
- Row locks are written **into the row on disk** (`SELECT FOR UPDATE` causes writes!), which
  ...
  and why they're invisible in `pg_locks` ([monitoring lesson](/03-locking/monitoring-locks)).
