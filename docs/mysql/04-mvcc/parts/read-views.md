...
```timeline
A: UPDATE balance=999 (+ commit)
R: first read → 999 (view opens here, not at BEGIN)
R: consistent snapshot — view opens now
A: UPDATE balance=111
R: read → 999 (snapshot predates it)
```
