<!-- GENERATED by `bun run gen` from scenarios/postgres/08-production/who-is-blocking-whom.yaml — do not edit by hand. -->

```timeline
A: UPDATE, then goes to lunch (idle in txn)
B: UPDATE balance = 300 → ⏳ waits
M: who blocks whom? → B waits on idle A
M: pg_terminate_backend(A)
