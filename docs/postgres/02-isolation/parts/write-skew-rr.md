...
```timeline
A: SELECT on_call → 2 ← "safe for me to leave"
B: SELECT on_call → 2 ← "safe for me to leave"
A: UPDATE alice off-call
B: UPDATE bob off-call (a different row)
A: COMMIT
B: COMMIT ← both succeed, no conflict to detect
A: SELECT on_call → 0 ← invariant broken
```
