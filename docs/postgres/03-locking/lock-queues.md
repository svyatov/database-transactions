...
The first: `C` waits on `B`, not on `A`. When `B` joined the queue it claimed a *tuple lock* —
its ticket for first place in line — and only then settled in to wait for A's transaction to
