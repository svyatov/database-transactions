...
```timeline
A: SELECT … FOR SHARE (A)
B: SELECT … FOR SHARE (B) → coexists
B: UPDATE → FOR NO KEY UPDATE → ⏳ waits
A: COMMIT (releases A's FOR SHARE)
B: ⏵ UPDATE → FOR NO KEY UPDATE → completes
A: UPDATE → FOR NO KEY UPDATE (A)
B: SELECT … FOR KEY SHARE → coexists with the UPDATE
B: SELECT … FOR UPDATE → ⏳ waits
A: COMMIT (releases A)
B: ⏵ SELECT … FOR UPDATE → completes
```
