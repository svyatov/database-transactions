...
- **No double-processing.** The claim is a [row lock](/postgres/03-locking/row-locks); `SKIP LOCKED`
...
- **No lost jobs.** B never wrote `state = 'running'` anywhere — its claim *was* its
  uncommitted transaction. When the worker dies, [atomicity](/postgres/01-basics/what-is-a-transaction)
...
- **The transaction is the lease.** A worker holds its connection and its transaction for
  the whole job — fine for seconds, wrong for hours. A [long transaction](/postgres/04-mvcc/long-transactions)
