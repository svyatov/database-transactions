...
<<< ../../scenarios/06-distributed/saga-compensation.ts#demo{ts}
...
- **Each step commits immediately.** There is no long-lived transaction holding
  [locks](/03-locking/row-locks) or [pinning VACUUM](/04-mvcc/long-transactions)
...
- **Compensation is not ROLLBACK.** `seats = seats + 1` is ordinary committed history —
  ...
  [idempotent](/05-patterns/idempotency)), and some steps — an email sent, cash
...
- Steps and compensations travel between services as messages, so the
  [outbox](/06-distributed/transactional-outbox) is the saga's transport: each step's
