...
The [outbox relay](/06-distributed/transactional-outbox) polls its table; polling
...
<<< ../../scenarios/06-distributed/listen-notify.ts#listener{ts}

## Three claims, one run

<<< ../../scenarios/06-distributed/listen-notify.ts#demo{ts}
...
- Delivery waits for COMMIT, so notifications from a long transaction arrive late. The
  manual's advice matches [chapter 4's](/04-mvcc/long-transactions):
...
- LISTEN and two-phase commit don't mix — a preview of the
  [next-door lesson](/06-distributed/two-phase-commit):
