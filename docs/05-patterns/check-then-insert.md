...
<<< ../../scenarios/05-patterns/check-then-insert-race.ts#demo{ts}
...
paths don't read each other's minds. (This is [write skew](/02-isolation/serializable)'s
...
<<< ../../scenarios/05-patterns/on-conflict.ts#demo{ts}
...
[transactionid wait you saw in lock queues](/03-locking/lock-queues) — and fails only
