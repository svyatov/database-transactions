...
[Chapter 2's scariest bug](/02-isolation/lost-update): two read-modify-write transactions,
...
<<< ../../scenarios/05-patterns/fix-lost-update-atomic.ts#demo{ts}

<!--@include: ./parts/fix-lost-update-atomic.md-->

B's UPDATE waits for A's row lock, then — this is the
[update-recheck behavior](/02-isolation/read-committed) from chapter 2, now working *for*
...
<<< ../../scenarios/05-patterns/fix-lost-update-for-update.ts#demo{ts}
...
[lock queues](/03-locking/lock-queues) with everything chapter 3 said about them.
...
<<< ../../scenarios/05-patterns/fix-lost-update-version-column.ts#demo{ts}
...
- The fourth fix is [REPEATABLE READ + retry](/05-patterns/retrying-serialization-failures):
