...
Chapter 3 explained [lock queues](/03-locking/lock-queues) and
[how to read pg_locks](/03-locking/monitoring-locks); this is the 3 a.m. version. One
...
<<< ../../scenarios/08-production/who-is-blocking-whom.ts#demo{ts}
...
took [that live until COMMIT](/03-locking/row-locks)) while the application forgot about
...
- An idle-in-transaction blocker can only be terminated, not canceled — and killing it
  ...
  [the next lesson](/08-production/long-and-idle-transactions) hunts those down.
- Termination is a rollback, and [a rollback is safe](/01-basics/what-is-a-transaction) —
