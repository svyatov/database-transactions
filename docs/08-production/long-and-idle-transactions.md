...
transaction. It [holds locks](/03-locking/row-locks), it
[pins VACUUM's horizon for the whole database](/04-mvcc/long-transactions), and it
occupies a pooled connection. This lesson is about finding those sessions, then making
sure they can't live long.

## Finding them

<<< ../../scenarios/08-production/find-long-transactions.ts#demo{ts}

<<< ../../scenarios/08-production/find-long-transactions.ts#detect{ts}

<!--@include: ./parts/find-long-transactions.md-->

Note what detector 3 proved: the report **wrote nothing** (`backend_xid` is null — no
transaction id was ever [assigned](/04-mvcc/row-versions)) and *still* pins the vacuum
...
<<< ../../scenarios/08-production/timeout-guardrails.ts#statement{ts}

<<< ../../scenarios/08-production/timeout-guardrails.ts#transaction{ts}

<<< ../../scenarios/08-production/timeout-guardrails.ts#aftermath{ts}
...
- [`idle_in_transaction_session_timeout`](https://www.postgresql.org/docs/current/runtime-config-client.html#GUC-IDLE-IN-TRANSACTION-SESSION-TIMEOUT)
  ...
  [chapter 5 proved it](/05-patterns/orm-pitfalls), server-side FATAL and all.
- [`transaction_timeout`](https://www.postgresql.org/docs/current/runtime-config-client.html#GUC-TRANSACTION-TIMEOUT)
  ...
  [chapter 6](/06-distributed/two-phase-commit):
