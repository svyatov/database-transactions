...
export default scenario({
  ...
  claim:
    "At READ COMMITTED, re-running the same range query inside one transaction can return rows that weren't there before — phantoms.",
  setup: `
  ...
  async run({ A, B }, t) {
    ...
    const [count] = await A`SELECT count(*) AS n FROM orders WHERE amount >= 100`;
    eq(count!.n, 2);

    t.note("Between A's two statements, B commits a new order that matches A's WHERE clause.");
    await B`INSERT INTO orders VALUES (3, 700)`;

    const [total] = await A`SELECT count(*) AS n, CAST(sum(amount) AS SIGNED) AS total FROM orders WHERE amount >= 100`;
