...
export default scenario({
  ...
  async run({ A, B }, t) {
    ...
    const [count] = await A`SELECT count(*)::int AS n FROM orders WHERE amount >= 100`;
    eq(count!.n, 2);
    ...
    const [total] = await A`SELECT count(*)::int AS n, sum(amount)::int AS total FROM orders WHERE amount >= 100`;
